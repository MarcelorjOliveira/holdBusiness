 ...
 * @ORM\Entity(repositoryClass="AppBundle\Repository\SymfonyUserRepository")
 */
class SymfonyUser implements UserInterface, \Serializable {
    ...
    public static function constroi($usuario) {
        ...
        $objeto->setProjects($objetoRepository->findBySymfonyUser($usuario->getId()));

        return $objeto;
    }

    public static function constroiPeloId($idSymfonyUser) {
        global $kernel;

        $manager = $kernel->getContainer()->get('doctrine.orm.entity_manager');

        $objetoRepository = $manager->getRepository('AppBundle:SymfonyUser');
        $usuario = $objetoRepository->find($idSymfonyUser);
        return $usuario;
    }
    ...
    public static function pesquisaPeloNome($pesquisa, $id) {
        ...
        //SymfonyUser::insereFotoPerfilObject($usuarios);

        $usuariosComStatus = Conexao::statusSymfonyUsers($id, $usuarios);
